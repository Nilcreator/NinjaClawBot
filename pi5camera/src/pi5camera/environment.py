...
_SYSTEM_IMPORT_FINDER_MARKER = "pi5camera-rpi-system-finder"
...
def _matches_system_target(
    fullname: str,
    module_names: tuple[str, ...] = _SYSTEM_IMPORT_TARGETS,
) -> bool:
    """Return whether ``fullname`` should be resolved from the system Python."""
    return any(fullname == name or fullname.startswith(f"{name}.") for name in module_names)


class _SystemModuleFinder(importlib.abc.MetaPathFinder):
    """Resolve selected Raspberry Pi packages from the system Python paths."""

    marker = _SYSTEM_IMPORT_FINDER_MARKER
    ...
    def find_spec(self, fullname: str, path=None, target=None):  # type: ignore[override]
        if not _matches_system_target(fullname, self.module_names):
            return None
...
def _has_system_module_finder() -> bool:
    return any(
        getattr(finder, "marker", None) == _SYSTEM_IMPORT_FINDER_MARKER for finder in sys.meta_path
    )


def _install_system_module_finder(
    system_paths: list[str],
    module_names: tuple[str, ...] = _SYSTEM_IMPORT_TARGETS,
) -> bool:
    """Install the targeted system-module finder into ``sys.meta_path``."""
    if not system_paths:
        return False
    if _has_system_module_finder():
        return True
    sys.meta_path.insert(0, _SystemModuleFinder(system_paths, module_names=module_names))
    importlib.invalidate_caches()
    return True


def _startup_helper_body(system_paths: list[str]) -> str:
    """Build a startup hook that resolves selected modules from system paths."""
    serialized_paths = json.dumps(system_paths)
    serialized_targets = json.dumps(list(_SYSTEM_IMPORT_TARGETS))
    return (
        ...
        f"MODULE_NAMES = tuple({serialized_targets})\n"
        f"MARKER = {json.dumps(_SYSTEM_IMPORT_FINDER_MARKER)}\n\n"
        "def _matches(fullname: str) -> bool:\n"
        "    return any(fullname == name or fullname.startswith(f'{name}.') for name in MODULE_NAMES)\n\n"
        "class _Pi5CameraSystemFinder(importlib.abc.MetaPathFinder):\n"
        "    marker = MARKER\n\n"
        "    def find_spec(self, fullname, path=None, target=None):\n"
        "        if not _matches(fullname):\n"
        "            return None\n"
        "        search_path = path if path is not None else list(SYSTEM_PATHS)\n"
        "        return importlib.machinery.PathFinder.find_spec(fullname, search_path)\n\n"
        "if not any(getattr(finder, 'marker', None) == MARKER for finder in sys.meta_path):\n"
        "    sys.meta_path.insert(0, _Pi5CameraSystemFinder())\n"
    )
...
def inject_system_site_packages(
    ...
    """Install a targeted system-module finder so Raspberry Pi packages are importable."""
    if not _is_virtual_environment():
        return False
    if platform.system() != "Linux":
        return False
    ...
    if not _install_system_module_finder(system_paths):
        return False
